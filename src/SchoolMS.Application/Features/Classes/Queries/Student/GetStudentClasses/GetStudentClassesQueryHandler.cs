...
public class GetStudentClassesQueryHandler(IAppDbContext context, IUser user) : IRequestHandler<GetStudentClassesQuery, Result<CursorResult<ClassDto>>>
{
    public async Task<Result<CursorResult<ClassDto>>> Handle(GetStudentClassesQuery query, CancellationToken cancellationToken)
    {
        var dbQuery = context.Classes.Where(c => c.StudentClasses.Any(sc => sc.StudentId == Guid.Parse(user.Id))).AsQueryable();
